...
- **`lib/backtest/holdout.ts`** — bounds verbatim: 2024-06-01T00:00Z inclusive → **2025-06-06T13:30Z exclusive** (verified against the DB: the dev set's first candle opens exactly at the exclusive bound). Ordinary runs CLIP holdout candles (loud warning) and refuse ranges entirely inside — exploration impossible by construction.
- **`lib/backtest/costs.ts`** — `FROZEN_COST_PROFILE_2026_07_18`: spread **0.26 persisted literally** per the user's formula max(floor 0.20, p95 NY AM observed). **Calibration deviation surfaced for review**: the stored ticks contain ZERO NY AM observations; the only window is London 2026-07-12 09:26→10:20 (n=3,158, spread constant 0.26 = p50 = p95 = p99). Using the 0.20 floor with 0.26 observed would be anti-conservative → 0.26. Collect real NY AM ticks before the verdict to recalibrate, else 0.26 stands. Stress profile 0.30/0.10 (informative only). `swap: null` + `rolloverCrossings`/`swapCostR` helpers (triple-swap Wednesday ×3).
- **`lib/backtest/verdict.ts`** — the user's bar encoded verbatim where quantified (`VERDICT_CRITERIA_2026_07_18`); **FAIL-first precedence** (test-pinned: exp +0.02R with one month at 67% of the total → FAIL, not INCONCLUSIVE — a fragile positive is a fail); seeded mulberry32 bootstrap, 10k iterations, seed 20260718 → verdict bit-reproducible. Proposed operationalizations flagged: "too wide" = CI width > 0.40R (a normal-variance n=150 breaches it on its own — the width rule effectively demands holdout-scale n≈300+), month rule defined only for a positive total, side rules at n ≥ 30.
- **Runner `--verdict-holdout`** — deliberately does NOT share the ordinary walk-forward path (auditable in isolation): rejects every override flag · refuses a dirty tree (verdict records the commit hash) · single read enforced by the `holdout_verdicts` PRIMARY KEY, not just code · sha256 of commit/dataset/candidate/cost-profile · zero metric output before the final block · attempts audited (`holdout_attempts`: started/completed/aborted_technical/refused_swap_invariant/refused_guard). **Swap invariant**: with `swap: null`, crossings are probed against BOTH 21:00 and 22:00 UTC (GMT+2/+3 server midnights); any crossing → refusal with NO metric computed, holdout stays virgin, audited. "Consumed" is defined by the DB row: technical failure before the verdict row exists → audited retry; row exists → forever.
- Schema: `cost_r`/`net_r_multiple` on trades (NULL on ordinary runs → gross-only display unchanged), `holdout_verdicts` (PK = window), `holdout_attempts`. Applied idempotently to the live DB.

Gates: **116 Vitest** (holdout 4, costs 9, verdict 12 incl. determinism + precedence), tsc 0, lint clean, dotnet 0/0 + 20/20. Guards verified behaviorally: override flags refused, dirty tree refused, clip inert on dev data.

**Heads-up recorded for the user**: NY AM entries (12:00–16:00 UTC) with the 32-bar horizon CAN cross 21:00/22:00 UTC — a first-attempt swap refusal is plausible. Options: provide Exness XAUUSDm swap rates + server rollover hour and re-freeze the profile (then commit and retry — audited), or accept the refusal as the invariant working. Next: user review checklist → import anterior history → commit → the single verdict run.
