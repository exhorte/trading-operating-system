...
(ADR 0010), construit jusqu'au mode CONFIRM uniquement.
...
| Chemin d'exécution | `lib/execution/`, `CockpitHub` | `RiskDecision → Command → ACK → Report`, mode `observe`/SIMULATED, idempotence validée en live. **Aucun appel de trade n'existe nulle part** — jusqu'à EA-02 (ADR 0010). |
...
**Chantier EA.** Le cadrage est écrit (ADR 0009, 0010, 0011 et fiche S01).
Avant toute ligne de code, trois vérifications listées en fin de S01 — le
spread réel contre un SL de 3 à 7 pips en tête, parce qu'il peut invalider le
régime de sortie retenu.

Ensuite, les fiches EA-01 à EA-06 : protocole d'exécution, agent MQL5,
réconciliation, profils de compte, policies de risque FTMO et réel, mode
CONFIRM.
