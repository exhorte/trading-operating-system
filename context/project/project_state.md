...
**User review before the verdict run**: bounds · cost profile (accept the London-proxy 0.26, or collect real NY AM ticks 12:00–16:00 UTC and re-freeze) · swap (provide Exness XAUUSDm rates + rollover hour, or accept refusal-on-crossing — NY AM entries + 32-bar horizon CAN cross 21:00/22:00 UTC, a first-attempt refusal is plausible) · bar operationalizations (width 0.40R, FAIL precedence, positive-total month rule). Then: import anterior history (safe) → commit → `npx tsx scripts/backtest.ts --verdict-holdout` → immutable verdict, whatever it says.
